/**
 * V2: Siembra Masiva de Datos para Esquema Multi-Almacén (Corregido)
 * ------------------------------------------------------------------
 * Script adaptado a la nueva estructura. Simula transacciones sobre
 * el Almacén Principal, actualizando la tabla 'inventario_por_almacen'.
 */
...
-- ===================================================================
-- SECCIÓN 3: GENERACIÓN MASIVA DE PRODUCTOS Y STOCK INICIAL
-- ===================================================================
DO $$
DECLARE
    admin_user_id INT := (SELECT id FROM usuarios WHERE nombre_usuario = 'admin');
...
BEGIN
    FOR i IN 1..50 LOOP
        ...
        VALUES ('ALG-' || LPAD(i::text, 4, '0'), 'Tela Jersey 30/1 Peinado, Color ' || i, precio_base, precio_base * 1.8, 50, cat_algodon, prov_sanjacinto, admin_user_id) ON CONFLICT (sku) DO NOTHING;
    END LOOP;
    FOR i IN 1..50 LOOP
    ...
    FOR new_product_id IN SELECT id FROM productos LOOP
        ...
        VALUES (new_product_id, alm_gamarra, floor(random() * 1000 + 200), admin_user_id, NOW())
        ON CONFLICT (producto_id, almacen_id) DO NOTHING;
    END LOOP;
END $$;

-- ===================================================================
-- SECCIÓN 4: SIMULACIÓN DE TRANSACCIONES
-- ===================================================================
DO $$
DECLARE
    i INT; j INT; k INT; num_sales INT; num_items INT;
...
BEGIN
    FOR i IN REVERSE 3650..1 LOOP
        FOR j IN 1..floor(random() * 14 + 2) LOOP
            ...
            FOR k IN 1..num_items LOOP
                ...
                WHERE inv.almacen_id = alm_principal_id AND inv.cantidad > 20 ORDER BY random() LIMIT 1;
        ...
        IF i % 7 = 0 THEN
            ...
            FOR k IN 1..num_items LOOP
                ...
                IF FOUND THEN
                    qty_bought := floor(random() * 200 + 50);
